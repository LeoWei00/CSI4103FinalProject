...
def standard_subspace_iteration(
    ...
    block_size = k + (1 if skip_trivial else 0)
...
def block_subspace_iteration(A, k, max_iter=1000, tol=1e-10, block_size=None):
    # Dense vs sparse
    if issparse(A):
        n = A.shape[0]
        I = sparse_eye(n, format="csr")
    ...
    # 1) Estimate largest eigenvalue of A
    w_max, _ = eigsh(A, k=1, which="LA")
    alpha = float(w_max[0])

    # 2) Define B = alpha I - A
    B = alpha * I - A
